...
const DEFAULT_CONFIG = {
  ...
  // 思维链（thinking）透传：CLI 的 thinking_delta 转发给客户端
  thinking: {
    // 有些上游不按 token 推思考，而是一次性给整段 —— 为了前台「逐字」体验，
    // 把这种整段切成小片匀速吐出去；真正的逐字增量（小片段）零延迟直接透传
    pace: true,
    maxChunk: 12, // 每片最多几个字
    tickMs: 18, // 每片间隔（毫秒）
    burstThreshold: 40, // 片段 ≤ 这个长度认为是真逐字，不加干预
  },
  openai: {
    // OpenAI 兼容接口里思维链放哪个字段：
    // reasoning_content（DeepSeek 系，默认）或 reasoning（OpenRouter 系）
    reasoningField: 'reasoning_content',
  },
  anthropic: {
    // 是否提供 /v1/messages（Anthropic 兼容接口，与 /v1/chat/completions 共存）
    enabled: true,
  },
  // 接口伪装：对外隐去「这是 wb-relay / WorkBuddy 反代」的一切痕迹。
  // 打开后：x-wb-* 响应头和响应体字段不再发送、/v1/models 只留标准字段、
  // 模型 id 的 cn/ intl/ 前缀被剥掉、/v1/wb/prompt 直接 404、错误信息里的
  // 本机路径和内部程序名被替换成占位符。伪装下会话名只认标准 user 字段。
  disguise: {
    enabled: false,
    // 伪装时 Server 响应头写这个值；留空 = 完全不发
    serverHeader: '',
  },
};
...
function loadConfig(args) {
  ...
  if (args.disguise === true) cfg.disguise.enabled = true;
...
用法: node wb-relay.js [选项]
  ...
  --disguise              开启接口伪装（隐去所有反代痕迹，等同 config.json 的 disguise.enabled）
...
接口:
  GET  /health
  GET  /v1/models
  POST /v1/chat/completions   OpenAI 兼容，支持 stream（可用 x-wb-site 点名站点），思维链在 reasoning_content
  POST /v1/messages           Anthropic 兼容，支持 stream，思考走 thinking 块 + signature
  POST /v1/wb/prompt          { prompt, session?, model?, site? } 简易接口（伪装模式下不可用）
`);
  process.exit(0);
}
...
// 从 assistant 事件的 content 数组里挑思考块（thinking / redacted_thinking）
function extractThinking(ev) {
  ...
  for (const c of content) {
    ...
    if (c.type === 'thinking' && typeof c.thinking === 'string') {
      return { text: c.thinking, signature: typeof c.signature === 'string' ? c.signature : '' };
    }
    if (c.type === 'redacted_thinking' && typeof c.data === 'string') {
      return { text: '', signature: '', redacted: c.data };
    }
  }
  return null;
}

// 思考「匀速器」：上游整段给思考时切成小片匀速吐，真逐字的小片段零延迟直接透传。
// 缺了它，前台表现就是「长时间无输出 → 整块思考弹出」。
function makeThinkingPacer(emit, { pace = true, maxChunk = 12, tickMs = 18, burstThreshold = 40 } = {}) {
  if (pace === false) return { push: (t) => { if (t) emit(t); }, finish: () => {} };
  let queue = '';
  let timer = null;
  const flush = () => {
    timer = null;
    if (!queue) return;
    const piece = queue.slice(0, maxChunk);
    queue = queue.slice(maxChunk);
    emit(piece);
    if (queue) timer = setTimeout(flush, tickMs);
  };
  return {
    push(text) {
      if (!text) return;
      if (queue) {
        // 正在匀速吐，后面的都接上
        queue += text;
        if (!timer) timer = setTimeout(flush, tickMs);
        return;
      }
      if (text.length <= burstThreshold) { emit(text); return; } // 真逐字，直接透传
      queue = text;
      timer = setTimeout(flush, tickMs);
    },
    // 正文到来前和收尾时必须调，把队列吐干净 —— 否则思考与正文的顺序会乱，甚至丢内容
    finish() {
      if (timer) { clearTimeout(timer); timer = null; }
      if (queue) { const rest = queue; queue = ''; emit(rest); }
    },
  };
}
...
class Worker {
  ...
  onEvent(ev, onReady) {
    ...
    // 正文在 content_block_delta / text_delta 里；思考过程在 thinking_delta 里，
    // 签名在 signature_delta 里（Anthropic 客户端要靠它接受后续轮次，必须实时转发）。
    if (ev.type === 'stream_event') {
      const e = ev.event;
      if (!e) return;
      if (e.type === 'content_block_start' && e.content_block && e.content_block.type === 'thinking') {
        p.thinkingIndex = typeof e.index === 'number' ? e.index : 0;
        return;
      }
      if (e.type === 'content_block_delta' && e.delta) {
        if (e.delta.type === 'thinking_delta' && e.delta.thinking) {
          p.liveThinking += e.delta.thinking;
          if (p.onThinking) p.onThinking(e.delta.thinking);
          return;
        }
        if (e.delta.type === 'signature_delta' && e.delta.signature) {
          p.thinkingSignature += e.delta.signature;
          if (p.onSignature) p.onSignature(p.thinkingSignature); // 传累计值，由调用方算增量
          return;
        }
    ...
    if (ev.type === 'assistant') {
      // 思考快照：整段模式（没开逐字增量）下思考只在这里出现；
      // 逐字模式下它是已发内容的完整版，只补漏掉的尾巴，别重复发
      const th = extractThinking(ev);
      if (th) {
        if (th.text && th.text.length > p.liveThinking.length && th.text.startsWith(p.liveThinking)) {
          const tail = th.text.slice(p.liveThinking.length);
          p.liveThinking = th.text;
          if (tail && p.onThinking) p.onThinking(tail);
        }
        if (th.signature && th.signature !== p.thinkingSignature) {
          p.thinkingSignature = th.signature;
          if (p.onSignature) p.onSignature(th.signature);
        }
      }
  ...
  // 一段对话里同一时刻只跑一轮，其余排队
  ask(input, { onDelta, onThinking, onSignature }) {
    const run = () => this._askOne(input, { onDelta, onThinking, onSignature });
    const next = this.queue.then(run, run);
    this.queue = next.catch(() => {});
    return next;
  }

  async _askOne(input, { onDelta, onThinking, onSignature } = {}) {
    await this.start();
...
// Anthropic 的 content block → CLI 的 content：text 直接转，image 是 {source:{type:'base64',...}}
function anthContentToCli(content) {
  if (typeof content === 'string') return [{ type: 'text', text: content }];
  if (!Array.isArray(content)) return [{ type: 'text', text: String(content ?? '') }];
  const out = [];
  for (const part of content) {
    if (!part || typeof part !== 'object') continue;
    if (part.type === 'text' && typeof part.text === 'string') out.push({ type: 'text', text: part.text });
    else if (part.type === 'image' && part.source && part.source.type === 'base64' && part.source.data) {
      out.push({ type: 'image', source: { type: 'base64', media_type: part.source.media_type || 'image/png', data: part.source.data } });
    }
  }
  if (!out.length) out.push({ type: 'text', text: '' });
  return out;
...
/* ------------------------------------------------------------------ *
 * 接口伪装：对外隐去「这是 wb-relay / WorkBuddy 反代」的痕迹
 * ------------------------------------------------------------------ */

function disguiseOn() {
  return Boolean(cfg.disguise && cfg.disguise.enabled);
}

// 模型 id 对外怎么写：伪装下剥掉 cn/ intl/ 站点前缀（前缀本身就是破绽）
function publicModelId(id) {
  let s = String(id || '');
  if (disguiseOn()) s = s.replace(/^(cn|intl)\s*\//i, '');
  return s;
}

// 会话名从哪来：伪装下不再读 x-wb-session 这种私有头，只认标准字段
function sessionNameOf(req, body) {
  if (disguiseOn()) {
    if (typeof body.user === 'string' && body.user.trim()) return body.user.trim();
    const meta = body.metadata && typeof body.metadata.user_id === 'string' ? body.metadata.user_id.trim() : '';
    return meta || 'default';
  }
  return sessionKeyOf(req, body);
}

// 站点归属的响应头 / 响应体字段：伪装下完全不发（这些字段名本身就是破绽）
function siteHeadersFor(target, sessionName) {
...
function siteBodyFor(target, sessionName) {
  if (disguiseOn()) return {};
  return {
    'x-wb-site': target.site,
...
// 伪装时的 Server 响应头：配了才发，留空则完全不发
function serverHeaderObj() {
  if (!disguiseOn()) return {};
  const v = String((cfg.disguise && cfg.disguise.serverHeader) || '').trim();
  return v ? { server: v } : {};
}

// 错误信息脱敏：本机路径、项目名、内部程序名都替换成占位符
function scrubError(msg) {
  let s = String(msg || '');
  if (!disguiseOn()) return s;
  s = s.replace(/[A-Za-z]:[\\/][^\s"'，。；、）)】\]]+/g, '<path>');
  s = s.replace(/wb-?relay|workbuddy( ai)?|codebuddy/gi, 'relay');
  return s;
}

const errBody = (msg, type = 'invalid_request_error', code = null) => ({ error: { message: scrubError(msg), type, code } });

// Anthropic 风格的错误体：{ type: "error", error: { type, message } }
const anthErrBody = (msg, type = 'invalid_request_error') => ({ type: 'error', error: { type, message: scrubError(msg) } });
...
const server = http.createServer(async (req, res) => {
  ...
  if (url.pathname === '/health') {
    // 伪装下 /health 只回一句「活着」—— 版本号、路径、会话名全都是破绽
    if (disguiseOn()) return sendJson(res, 200, { status: 'ok' });
  ...
  if (url.pathname === '/v1/models' && req.method === 'GET') {
    ...
    // 伪装：只留标准字段，owned_by 用中性值，模型 id 剥掉站点前缀，顶层不带任何 _ 开头的私有字段
    if (disguiseOn()) {
      return sendJson(res, 200, {
        object: 'list',
        data: picked.map((m) => ({ id: publicModelId(m.exposedId), object: 'model', created: 0, owned_by: 'system' })),
      });
    }
  ...
  // 伪装下这个私有路径直接不存在 —— 路径名里的 wb 本身就是破绽
  if (url.pathname === '/v1/wb/prompt' && req.method === 'POST' && !disguiseOn()) {
    let body;
  ...
  if (url.pathname === '/v1/chat/completions' && req.method === 'POST') {
    ...
    const sessionName = sessionNameOf(req, body);
    ...
    // 站点归属通过响应头/响应体告诉调用方（伪装模式下什么都不带）
    const siteBody = siteBodyFor(target, sessionName);
    const siteHeaders = siteHeadersFor(target, sessionName);

    // 客户端要不要思维链：include_reasoning / reasoning 显式给 false 才不要
    const wantThinking = body.include_reasoning !== false && body.reasoning !== false;
    const reasoningField = cfg.openai && cfg.openai.reasoningField === 'reasoning' ? 'reasoning' : 'reasoning_content';

    if (!stream) {
      try {
        const r = await worker.ask({ content: picked.content }, {});
        const message = { role: 'assistant', content: r.text };
        if (wantThinking && r.thinking) message[reasoningField] = r.thinking;
        return sendJson(res, 200, {
          id, object: 'chat.completion', created,
          model: publicModelId(requestedModel),
          ...
          ...siteBody,
          ...(disguiseOn() ? {} : { 'x-wb-cli-session': r.sessionId }),
        }, true, siteHeaders);
      } catch (e) {
    ...
    // SSE 实时性第一道：关掉 Nagle 攒包、立刻把响应头发出去，
    // 否则小数据包被 TCP 攒着，前台看到的是「卡住然后整块弹出」
    if (res.socket) res.socket.setNoDelay(true);
    if (typeof res.flushHeaders === 'function') res.flushHeaders();

    const chunk = (delta, finish = null) => ({
      id, object: 'chat.completion.chunk', created, model: publicModelId(requestedModel),
    ...
    // 思维链增量：第一个思考 delta 前先发一个空 delta，有些客户端靠它切 UI。
    // 整段给的思考走匀速器切成小片吐，保证前台是「逐字」而不是「整块弹出」。
    let thinkingStarted = false;
    const emitThinkingNow = (t) => {
      if (!t || !wantThinking) return;
      if (!thinkingStarted) { write(chunk({ [reasoningField]: '' })); thinkingStarted = true; }
      write(chunk({ [reasoningField]: t }));
    };
    const pacer = makeThinkingPacer(emitThinkingNow, cfg.thinking || {});
    ...
    try {
      const r = await worker.ask({ content: picked.content }, {
        onDelta: (d) => { pacer.finish(); write(chunk({ content: d })); },
        onThinking: (t) => pacer.push(t),
      });
      pacer.finish();
      // 整段模式兜底：思考只在最后的快照/结果里出现，一块都还没发过
      if (!thinkingStarted && wantThinking && r.thinking) {
        pacer.push(r.thinking);
        pacer.finish();
      }
      write(chunk({}, 'stop'));
      if (includeUsage && r.usage) {
        write({
          id, object: 'chat.completion.chunk', created, model: publicModelId(requestedModel), choices: [],
          usage: {
            prompt_tokens: r.usage.input_tokens || 0,
            completion_tokens: r.usage.output_tokens || 0,
            total_tokens: (r.usage.input_tokens || 0) + (r.usage.output_tokens || 0),
      ...
      pacer.finish();
      write({ error: { message: scrubError(e.message), type: 'server_error' } });
      if (!res.writableEnded) res.write('data: [DONE]\n\n');
    } finally {
  ...
  /* ---------- Anthropic 兼容层（与 OpenAI 接口共存） ---------- */

  if (url.pathname === '/v1/messages' && req.method === 'POST') {
    if (cfg.anthropic && cfg.anthropic.enabled === false) {
      return sendJson(res, 404, anthErrBody('未知路径：' + url.pathname, 'not_found_error'));
    }
    let body;
    try { body = await readBody(req); } catch (e) { return sendJson(res, 400, anthErrBody(e.message)); }

    // Anthropic 没有 system 角色：system 是顶层字段，可以是 string 或 block 数组
    let systemPrompt = '';
    if (typeof body.system === 'string') systemPrompt = body.system;
    else if (Array.isArray(body.system)) {
      systemPrompt = body.system.map((b) => (b && typeof b.text === 'string') ? b.text : '').filter(Boolean).join('\n');
    }

    const messages = Array.isArray(body.messages) ? body.messages : [];
    let lastUser = null;
    for (const m of messages) if (m && m.role === 'user') lastUser = m;
    if (!lastUser) return sendJson(res, 400, anthErrBody('messages 里没有 user 角色的内容'));
    // max_tokens 在 Anthropic 里是必填，这里只记录、不限制（长度由模型自己决定）
    const maxTokens = Number(body.max_tokens) || 0;

    if (!activeSites.length) return sendJson(res, 503, anthErrBody(notReadyError(), 'api_error'));

    const sessionName = sessionNameOf(req, body);
    const requestedModel = typeof body.model === 'string' && body.model ? body.model : 'workbuddy-auto';
    const target = resolveTarget(requestedModel, req.headers['x-wb-site'] || body.site || '');
    if (target.error) return sendJson(res, 400, anthErrBody(target.error));
    const workerKey = workerIdOf(target.site, sessionName, target.model);

    let worker;
    try {
      const freshSystem = (!workers.has(workerKey) && systemPrompt) ? systemPrompt : '';
      worker = getWorker(workerKey, freshSystem || cfg.cli.systemPrompt, target.model, target.site);
    } catch (e) {
      return sendJson(res, 429, anthErrBody(e.message, 'rate_limit_error'));
    }

    const msgId = 'msg_' + crypto.randomUUID().replace(/-/g, '').slice(0, 24);
    const content = anthContentToCli(lastUser.content);
    const stream = body.stream === true;
    const siteHeaders = siteHeadersFor(target, sessionName);
    const usageOf = (r) => ({
      input_tokens: (r && r.usage && r.usage.input_tokens) || 0,
      output_tokens: (r && r.usage && r.usage.output_tokens) || 0,
    });

    if (!stream) {
      try {
        const r = await worker.ask({ content }, {});
        const blocks = [];
        if (r.thinking) blocks.push({ type: 'thinking', thinking: r.thinking, signature: r.thinkingSignature || '' });
        ...
        log(`[/v1/messages] 出错：${e.message}`);
        return sendJson(res, 500, anthErrBody(e.message, 'api_error'));
      }
    }
    ...
    // SSE 每帧同时带 event: 名和 data: JSON，且 JSON 里的 type 与事件名一致
    const send = (name, obj) => { if (!res.writableEnded) res.write(`event: ${name}\ndata: ${JSON.stringify(obj)}\n\n`); };

    send('message_start', {
      type: 'message_start',
      message: {
        id: msgId, type: 'message', role: 'assistant', content: [],
        model: publicModelId(requestedModel), stop_reason: null, stop_sequence: null,
        usage: { input_tokens: 0, output_tokens: 0 },
      },
    });

    // 块 index 必须递增：思考 0、正文 1（没有思考时正文就是 0）。
    // 思考块懒开：第一个思考增量到来才开，正文到来前必须关掉。
    let blockIndex = 0;
    ...
    const openThinking = () => {
      if (thinkingOpen || textOpen) return; // 思考只能在正文之前
    ...
    const emitThinkingNow = (t) => {
      if (!t) return;
      openThinking();
      if (!thinkingOpen) return; // 正文已经开始了，思考没地方放
      thinkingSent = true;
      send('content_block_delta', { type: 'content_block_delta', index: 0, delta: { type: 'thinking_delta', thinking: t } });
    };
    const pacer = makeThinkingPacer(emitThinkingNow, cfg.thinking || {});

    // signature 必须实时转发（客户端靠它接受后续轮次）；onSignature 给的是累计值，这里算增量
    const onSignature = (sig) => {
      if (!sig || !thinkingOpen) return;
      const delta = sig.startsWith(sentSignature) ? sig.slice(sentSignature.length) : sig;
      if (!delta) return;
      sentSignature = sig.startsWith(sentSignature) ? sig : sentSignature + sig;
      send('content_block_delta', { type: 'content_block_delta', index: 0, delta: { type: 'signature_delta', signature: delta } });
    };
    ...
    try {
      const r = await worker.ask({ content }, {
        onDelta: (d) => {
          pacer.finish(); // 正文到来前把思考队列吐干净，顺序不能乱
          openText();
          send('content_block_delta', { type: 'content_block_delta', index: textIndex, delta: { type: 'text_delta', text: d } });
        },
        onThinking: (t) => pacer.push(t),
        onSignature,
      });
      pacer.finish();
      // 整段模式兜底：思考只在快照/结果里出现，一块都还没发过
      if (!thinkingSent && r.thinking) {
        pacer.push(r.thinking);
        pacer.finish();
        if (r.thinkingSignature) onSignature(r.thinkingSignature);
      }
      // 正文一块都没发过（空回复）也要给个空 text 块，不然事件序列不完整
      openText();
      send('content_block_stop', { type: 'content_block_stop', index: textIndex });
      send('message_delta', {
        type: 'message_delta',
        delta: { stop_reason: 'end_turn', stop_sequence: null },
        usage: { output_tokens: usageOf(r).output_tokens },
      });
      send('message_stop', { type: 'message_stop' });
    } catch (e) {
      log(`[/v1/messages:stream] 出错：${e.message}`);
      pacer.finish();
      send('error', { type: 'error', error: { type: 'api_error', message: scrubError(e.message) } });
    } finally {
      clearInterval(heartbeat);
      if (!res.writableEnded) res.end();
    }
...
server.listen(cfg.listen.port, cfg.listen.host, () => {
  ...
  console.log(`  思维链    透传开启（OpenAI 接口在 ${cfg.openai && cfg.openai.reasoningField === 'reasoning' ? 'reasoning' : 'reasoning_content'}，Anthropic 接口走 thinking 块）`);
  console.log(`  Anthropic ${cfg.anthropic && cfg.anthropic.enabled === false ? '接口已关闭' : '/v1/messages 已提供（与 /v1/chat/completions 共存）'}`);
  console.log(`  接口伪装  ${disguiseOn() ? '已开启 —— 对外不暴露任何 wb-relay / WorkBuddy 痕迹' : '关闭（config.json 的 disguise.enabled 或 --disguise 可打开）'}`);
